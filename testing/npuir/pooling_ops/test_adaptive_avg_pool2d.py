...
"""AdaptiveAvgPool2d operator test (Developer mode)."""
...
pytestmark = [
    pytest.mark.op("adaptive_avg_pool2d"),
...
def adaptive_avg_pool2d_kernel(
    H_in, W_in, H_out, W_out, dtype="float16", accum_dtype="float32"
):
    """Adaptive average pool 2d kernel (uniform-split strategy).
    ...
    standard average pool with stride = kernel = H_in // H_out.
    """
    assert H_in % H_out == 0, (
    ...
    stride_h = H_in // H_out
    stride_w = W_in // W_out
    kernel_area = float(stride_h * stride_w)
    inv_area = 1.0 / kernel_area
    ...
    def kernel_fn(
        ...
        with T.Kernel(1, is_npu=True) as (cid, _):
            ...
            in_f32 = T.alloc_shared((H_in, W_in), accum_dtype)
            acc_f32 = T.alloc_shared((H_out, W_out), accum_dtype)

            # 1. Load entire input (same dtype)
            T.copy(In, in_sh)

            # 2. Cast input → f32 element-wise
            for i, j in T.Parallel(H_in, W_in):
                in_f32[i, j] = T.cast(in_sh[i, j], accum_dtype)

            # 3. Sliding-window accumulation (no if inside T.serial)
            for oi, oj in T.Parallel(H_out, W_out):
                hi = oi * stride_h
                wi = oj * stride_w

                # ---- window element (0, 0) ----
                acc_f32[oi, oj] = in_f32[hi, wi]

                # ---- kh=0, kw=1..stride_w-1 ----
                for kj in T.serial(stride_w - 1):
                    acc_f32[oi, oj] = acc_f32[oi, oj] + in_f32[hi, wi + kj + 1]

                # ---- kh=1..stride_h-1, kw=0..stride_w-1 ----
                for ki in T.serial(stride_h - 1):
                    for kj in T.serial(stride_w):
                        acc_f32[oi, oj] = acc_f32[oi, oj] + in_f32[hi + ki + 1, wi + kj]

            # 4. SIMD division to obtain the mean
            T.vmul(acc_f32, inv_area, acc_f32)

            # 5. Cast back → dtype element-wise
...
def _ref_adaptive_avg_pool2d(x: torch.Tensor, H_out: int, W_out: int) -> torch.Tensor:
    """Reference via ``torch.nn.functional.adaptive_avg_pool2d``.

    F.adaptive_avg_pool2d expects (N, C, H, W) input; we lift the 2-d
    ...
    ref_4d = F.adaptive_avg_pool2d(x_4d.float(), (H_out, W_out))
...
def _compile_and_run(H_in, W_in, H_out, W_out, dtype, rtol=1e-2, atol=1e-2):
    ...
    ref = _ref_adaptive_avg_pool2d(x.cpu(), H_out, W_out)

    func = adaptive_avg_pool2d_kernel(
        H_in=H_in,
        W_in=W_in,
        H_out=H_out,
        W_out=W_out,
